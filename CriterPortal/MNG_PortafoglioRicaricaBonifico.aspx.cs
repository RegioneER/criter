...
public partial class MNG_PortafoglioRicaricaBonifico : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        SecurityManager.CheckPagePermissions(HttpContext.Current.User.Identity.Name, "MNG_PortafoglioRicaricaBonifico.aspx");
        cvDataBonifico.ValueToCompare = DateTime.Today.AddDays(-30).ToString("dd/MM/yyyy");
    ...
    {
        cvDataBonifico.Enabled = !chkDisableValidationDate.Checked;
    }

    protected void btnBonifico_Click(Object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            decimal importo = 0;
            ...
            if (decimal.TryParse(this.txtImportoBonifico.Text, out importo))
            {
                if (ASPxComboBox1.Value != null)
                {
                    ...
                    if (portafoglio != null)
                    {
                        using (var ctx = new CriterDataModel())
                        {
                            ...
                            DateTime dt = DateTime.Parse(txtDataBonifico.Text);
                            decimal QuantitaBollini;
                            QuantitaBollini = Math.Floor(importo / CostoBollino);
                            quantita = int.Parse(QuantitaBollini.ToString());

                            var rp = portafoglio.Incasso(azienda, DateTime.Now, importo, quantita, dt, txtCausale.Text);
                            portafoglio.Save();
